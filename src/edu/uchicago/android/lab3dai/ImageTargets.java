...
package edu.uchicago.android.lab3dai;
...
import edu.uchicago.android.lab3dai.R;
...
public class ImageTargets extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			mTextures.add(Texture.loadTexture(am.open("TextureDefault.png", AssetManager.ACCESS_BUFFER)));
		}
		catch (Exception e) {
		...
		// Connect to wunderground.com to obtain weather forecast json
		HttpClient hc = new DefaultHttpClient();

		try {
			//make an API call for json return
			HttpResponse response = hc.execute(new HttpGet(
					"http://api.wunderground.com/api/d60ec34648d43b43/forecast/q/autoip.json"));
			Log.d("HTTP response", response.getEntity().getContentType()
					.toString());

			// buffer the content of HTTP response
			String temp;
			String data = "";
			...
			//load the corresponding weather icons into textures
			String icon_url = json.getJSONObject("forecast").getJSONObject("txt_forecast").getJSONArray("forecastday").getJSONObject(0).getString("icon_url");
			Log.d("Json", icon_url);
			
			//get the weather icon
			response = hc.execute(new HttpGet(icon_url));
			
			//add the icon to texture 
			mTextures.add(Texture.loadTexture(response.getEntity().getContent()));
			
			//tomorrow's weather
			//load the corresponding weather icons into textures
			//wunderground returns a 12 hr period forecast
			if (json.getJSONObject("forecast").getJSONObject("txt_forecast").getJSONArray("forecastday").getJSONObject(1).getString("title") == "Tonight") {
				icon_url = json.getJSONObject("forecast").getJSONObject("txt_forecast").getJSONArray("forecastday").getJSONObject(2).getString("icon_url");
			}
			else {
				icon_url = json.getJSONObject("forecast").getJSONObject("txt_forecast").getJSONArray("forecastday").getJSONObject(1).getString("icon_url");
			}
			Log.d("Json", icon_url);
			...
			//change the bFetch flag
			bFetch = true;
		} 
		catch (Exception e) {
			Log.e("HTTP response and JSON", e.toString());
			Toast.makeText(this, "Failed to obtain weather information.",
	...
	private void initApplicationAR() {
		...
		mRenderer = new ImageTargetsRenderer(bFetch);
	...
	protected Dialog onCreateDialog(int id) {
		...
		if (id == DIALOG_ABOUT) {
			...
			((Button)about.findViewById(R.id.done_btn)).setOnClickListener(
					new OnClickListener() {
						public void onClick(View v) {
							ImageTargets.this.dismissDialog(DIALOG_ABOUT);
						}
					});
		}
		return about;
